...
class ReminderScheduler:
    ...
    def reminder_service(self):
        ...
        if self._reminder_service is None:
            from services.optimization_reminder_service import (
                OptimizationReminderService,
            )

            self._reminder_service = OptimizationReminderService(
                db=self.db, telegram_service=self.telegram_service
            )
    ...
    async def _update_digest_schedule(self):
        ...
        try:
            ...
            if self.scheduler:
                ...
                logger.info(
                    f"[SCHEDULER] Updated digest schedule: {day} at {hour:02d}:{minute:02d}"
                )
        except Exception as e:
    ...
    async def trigger_digest_now(self) -> dict:
        ...
        return {
            "message": "Digest job triggered manually",
            "timestamp": datetime.now(timezone.utc).isoformat(),
        }
...
def init_reminder_scheduler(
    db: AsyncIOMotorDatabase, telegram_service=None
) -> ReminderScheduler:
