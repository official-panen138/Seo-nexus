...
class ActivityLogService:
    ...
    async def get_logs(
        ...
        if entity_type:
            query["entity_type"] = entity_type.value if isinstance(entity_type, EntityType) else entity_type
        if entity_id:
            query["entity_id"] = entity_id
        if actor:
            query["actor"] = actor
        if action_type:
            query["action_type"] = action_type.value if isinstance(action_type, ActionType) else action_type
    ...
    def _sanitize_for_storage(self, data: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """
