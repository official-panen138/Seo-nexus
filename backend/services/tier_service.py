...
class TierCalculationService:
    ...
    async def calculate_domain_tier(
        self, network_id: str, entry_id: str
    ) -> Tuple[int, str]:
        """
    ...
    async def get_tier_by_asset_domain(
        self, network_id: str, asset_domain_id: str
    ) -> Tuple[int, str]:
        """
        ...
        entry = await self.db.seo_structure_entries.find_one(
            {"network_id": network_id, "asset_domain_id": asset_domain_id},
            {"_id": 0, "id": 1},
        )
    ...
    async def get_entries_at_tier(self, network_id: str, tier: int) -> List[str]:
        """
        ...
        tiers = await self.calculate_network_tiers(network_id)
        return [
            entry_id for entry_id, entry_tier in tiers.items() if entry_tier == tier
        ]
    ...
    async def validate_hierarchy(self, network_id: str) -> Dict[str, List[str]]:
        ...
        issues = {"orphans": [], "cycles": [], "multiple_mains": []}
        ...
        for entry in entries:
            ...
            if entry.get("domain_role") != DomainRole.MAIN.value:
                if tiers.get(entry_id, self.MAX_TIER) == self.MAX_TIER:
                    ...
                    if not entry.get("target_entry_id") and not entry.get(
    ...
    async def enrich_entries_with_tiers(
        self, entries: List[dict], network_id: Optional[str] = None
    ) -> List[dict]:
        """
