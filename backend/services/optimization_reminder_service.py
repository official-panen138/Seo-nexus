...
class OptimizationReminderService:
    ...
    async def get_global_reminder_config(self) -> Dict[str, Any]:
        ...
        if settings:
            return {
                ...
                "interval_days": settings.get("interval_days", self.DEFAULT_REMINDER_INTERVAL_DAYS)
            }
    ...
    async def get_network_reminder_config(self, network_id: str) -> Optional[Dict[str, Any]]:
        """Get per-network reminder override if configured"""
        network = await self.db.seo_networks.find_one(
            {"id": network_id},
            {"_id": 0, "reminder_config": 1}
        )
    ...
    async def get_in_progress_optimizations_needing_reminder(self) -> List[Dict[str, Any]]:
        """
        ...
        logger.info(f"[REMINDER] Found {len(need_reminder)} optimizations needing reminder")
        return need_reminder
    
    async def get_users_to_notify(self, optimization: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        ...
        if user_ids:
            users = await self.db.users.find(
                {"id": {"$in": list(user_ids)}, "status": {"$nin": ["inactive", "suspended"]}},
    ...
    async def send_reminder(self, optimization: Dict[str, Any], users: List[Dict[str, Any]]) -> bool:
    ...
    async def log_reminder(
        ...
        logger.info(f"[REMINDER] Logged reminder {log_entry['id']} for optimization {optimization['id']}")
    ...
    async def process_all_reminders(self) -> Dict[str, Any]:
        ...
        for opt in optimizations:
            try:
                ...
                if not users:
                    logger.info(f"[REMINDER] No users to notify for optimization {opt['id']}")
                    continue
                ...
                logger.error(f"[REMINDER] Failed to process optimization {opt['id']}: {e}")
                results["reminders_failed"] += 1
...
async def add_reminder_method_to_telegram_service():
    ...
    async def send_in_progress_reminder(
        ...
        try:
            ...
            tags_text = "\n".join([f"  • {tag}" for tag in tags]) if tags else "  (No users tagged)"
