...
def format_to_local_time(
    dt_input, timezone_str: str = "Asia/Jakarta", timezone_label: str = "GMT+7"
) -> str:
    """
    ...
    try:
        ...
        return (
            f"{local_dt.strftime('%Y-%m-%d %H:%M')} {timezone_label} ({timezone_str})"
        )
...
def format_now_local(
    timezone_str: str = "Asia/Jakarta", timezone_label: str = "GMT+7"
) -> str:
    """Get current time formatted in local timezone"""
    return format_to_local_time(
        datetime.now(timezone.utc), timezone_str, timezone_label
    )
...
TIMEZONE_OPTIONS = [
    ...
    },
    {
        ...
        "label": "GMT-8 (America/Los_Angeles)",
        "offset": "-08:00",
    },
