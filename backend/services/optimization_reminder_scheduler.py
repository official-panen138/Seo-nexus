...
class OptimizationReminderScheduler:
    ...
    async def get_global_reminder_config(self) -> Dict[str, Any]:
        ...
        config = await self.db.settings.find_one(
            {"key": "reminder_config"},
            {"_id": 0}
        )
        return config or {
    ...
    async def get_effective_reminder_interval(self, network_id: str) -> int:
        ...
        return global_config.get("default_interval_days", DEFAULT_REMINDER_INTERVAL_DAYS)
    ...
    async def should_send_reminder(self, optimization: Dict[str, Any]) -> bool:
        ...
        # Check last activity
        last_activity = optimization.get("last_activity_at") or optimization.get("created_at")
        if last_activity:
    ...
    async def send_reminder(self, optimization: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not network:
                logger.warning(f"Network not found for optimization {optimization['id']}")
                return False
            ...
            if success:
                ...
                await self.db.seo_optimizations.update_one(
                    {"id": optimization["id"]},
                    {"$set": {"last_reminded_at": datetime.now(timezone.utc).isoformat()}}
                )
            ...
            logger.error(f"Failed to send reminder for optimization {optimization['id']}: {e}")
            return False
        ...
        try:
            ...
            async for optimization in cursor:
                ...
                try:
                    ...
                    logger.error(f"Error processing optimization {optimization.get('id')}: {e}")
                    stats["errors"] += 1
    ...
    async def start_scheduler(self, interval_hours: int = 6):
        ...
        """
        self._running = True
        logger.info(f"Starting optimization reminder scheduler (interval: {interval_hours}h)")
