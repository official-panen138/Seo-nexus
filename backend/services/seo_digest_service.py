...
class SeoDigestService:
    ...
    async def generate_weekly_digest(
        ...
        return {
            ...
            "complaint_resolution_rate": round(resolved_complaints / total_complaints * 100, 1) if total_complaints > 0 else 0,
    ...
    async def format_telegram_digest(self, digest: Dict[str, Any]) -> str:
        ...
        if digest['by_status']:
            lines.append("*Status:*")
            for status, count in sorted(digest['by_status'].items(), key=lambda x: x[1], reverse=True):
        ...
        if digest['by_activity_type']:
            ...
            for activity_type, count in sorted(digest['by_activity_type'].items(), key=lambda x: x[1], reverse=True)[:5]:
        ...
        if digest['top_performers']:
            ...
            for i, performer in enumerate(digest['top_performers'][:3], 1):
                ...
                lines.append(f"  {medal} {performer['name']}: {performer['count']} selesai")
            lines.append("")
        ...
        if digest['total_complaints'] > 0:
            ...
            lines.append(f"  • Tingkat Resolusi: {digest['complaint_resolution_rate']}%")
            lines.append("")
        
        # Network breakdown
        if digest['by_network']:
            lines.append("🌐 *Per Network:*")
            for network, count in sorted(digest['by_network'].items(), key=lambda x: x[1], reverse=True)[:5]:
    ...
    async def send_digest(
        ...
        try:
            ...
            settings = await self.db.settings.find_one(
                {"_id": "seo_optimization_telegram"},
                {"_id": 0}
            )
            
            if not settings or not settings.get("enabled") or not settings.get("bot_token") or not settings.get("chat_id"):
