...
class WeeklyDigestService:
    ...
    async def _collect_expiring_domains(self, days_threshold: int = 30) -> Dict[str, List[Dict]]:
        """
    ...
    async def _collect_down_domains(self) -> List[Dict]:
        ...
        for domain in domains:
            ...
            # Get last check time
            domain["last_checked"] = domain.get("last_checked_at") or domain.get("last_check")
    ...
    def _format_digest_email(self, data: Dict[str, Any]) -> tuple:
        ...
        subject = f"[SEO-NOC] Weekly Domain Health Digest - {health_emoji} {health_status}"
        ...
        if expiring.get("total", 0) > 0:
            ...
            if expiring.get("critical"):
                ...
                for d in expiring["critical"][:10]:
                    seo_badge = f'<span style="background: #1e3a5f; color: #60a5fa; padding: 2px 6px; border-radius: 3px; font-size: 10px;">{d.get("seo_networks_count", 0)} networks</span>' if d.get("seo_networks_count", 0) > 0 else '<span style="color: #6b7280; font-size: 10px;">No SEO</span>'
                    days_text = f'{d.get("days_remaining")} days' if d.get("days_remaining", 0) >= 0 else f'EXPIRED {abs(d.get("days_remaining", 0))}d ago'
                    exp_rows += f'''
                    <tr>
        ...
        if soft_blocked:
            ...
            for d in soft_blocked[:15]:
                seo_badge = f'<span style="background: #1e3a5f; color: #60a5fa; padding: 2px 6px; border-radius: 3px; font-size: 10px;">{d.get("seo_networks_count", 0)} networks</span>' if d.get("seo_networks_count", 0) > 0 else '<span style="color: #6b7280; font-size: 10px;">No SEO</span>'
                block_label = block_type_labels.get(d.get("block_type", ""), d.get("block_type", "Unknown"))
                blocked_rows += f'''
                <tr>
                    <td style="padding: 8px; border-bottom: 1px solid #333; font-family: monospace;">{d.get("domain_name", "N/A")}</td>
                    <td style="padding: 8px; border-bottom: 1px solid #333;">{d.get("brand_name", "N/A")}</td>
    ...
    async def generate_and_send_digest(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "issues_count": data.get("expiring_domains", {}).get("total", 0) + len(data.get("down_domains", [])) + len(data.get("soft_blocked_domains", []))
            }
    ...
    async def preview_digest(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_issues": data.get("expiring_domains", {}).get("total", 0) + len(data.get("down_domains", [])) + len(data.get("soft_blocked_domains", []))
        }
