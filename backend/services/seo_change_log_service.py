...
class SeoChangeLogService:
    ...
    async def get_network_change_history(
        ...
        logs = (
            await self.change_logs.find(query, {"_id": 0})
            .sort("created_at", -1)
            .skip(skip)
            .limit(limit)
            .to_list(limit)
    ...
    async def get_network_notifications(
        self, network_id: str, unread_only: bool = False, skip: int = 0, limit: int = 20
    ) -> List[Dict[str, Any]]:
        ...
        notifications = (
            await self.notifications.find(query, {"_id": 0})
            .sort("created_at", -1)
            .skip(skip)
            .limit(limit)
            .to_list(limit)
    ...
    async def archive_old_logs(self, days_old: int = 90) -> int:
        ...
        result = await self.change_logs.update_many(
            {"created_at": {"$lt": cutoff.isoformat()}, "archived": {"$ne": True}},
            {
                "$set": {
                    "archived": True,
    ...
    async def get_team_stats(
        self, brand_ids: Optional[List[str]] = None, days: int = 30
    ) -> Dict[str, Any]:
        ...
        changes_by_network = await self.change_logs.aggregate(network_pipeline).to_list(
            100
        )
        ...
        domain_pipeline = [
            {"$match": query},
            {"$group": {"_id": "$affected_node", "change_count": {"$sum": 1}}},
            {"$sort": {"change_count": -1}},
            {"$limit": 20},
        ]
