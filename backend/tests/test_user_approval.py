...
class TestUserApprovalWorkflow:
    ...
    def brands(self, auth_headers):
        ...
        response = requests.get(
            f"{BASE_URL}/api/brands?include_archived=false", headers=auth_headers
        )
        if response.status_code != 200:
            return []
    ...
    def test_register_new_user_returns_pending_status(self):
        ...
        assert (
            "pending" in data.get("message", "").lower()
            or "approval" in data.get("message", "").lower()
        )
    ...
    def test_pending_user_cannot_login(self):
        ...
        ), f"Expected 403 for pending user, got: {login_response.status_code}"
        data = login_response.json()
        assert (
            "awaiting" in data.get("detail", "").lower()
            or "approval" in data.get("detail", "").lower()
        )
    ...
    def test_get_pending_users_endpoint(self, auth_headers):
        ...
        assert (
            response.status_code == 200
        ), f"Failed to get pending users: {response.status_code}"
        data = response.json()
        ...
        for user in data:
            ...
            ), f"User {user.get('email')} has status {user.get('status')}"
    ...
    def test_approve_pending_user(self, auth_headers, brands):
        ...
        pending_response = requests.get(
            f"{BASE_URL}/api/users/pending", headers=auth_headers
        )
        pending_users = pending_response.json()

        target_user = next(
            (u for u in pending_users if u["email"] == unique_email), None
        )
        if not target_user:
        ...
        approve_response = requests.post(
            ...
            json={"role": "viewer", "brand_scope_ids": brand_ids},
        )
        ...
        assert (
            approved_user.get("brand_scope_ids") == brand_ids
        ...
        ), f"Approved user should be able to login, got: {login_response.status_code}"
    ...
    def test_reject_pending_user(self, auth_headers):
        ...
        reject_response = requests.post(
            f"{BASE_URL}/api/users/{user_id}/reject", headers=auth_headers
        )

        assert (
            reject_response.status_code == 200
        ), f"Reject failed: {reject_response.status_code} - {reject_response.text}"
        ...
        assert (
            login_response.status_code == 403
        ), f"Expected 403 for rejected user, got: {login_response.status_code}"
        data = login_response.json()
        assert (
            "rejected" in data.get("detail", "").lower()
        ), f"Expected 'rejected' in message: {data.get('detail')}"

        print(
            f"PASS: Rejected user {unique_email} correctly denied login with 'rejected' message"
        )
    ...
    def test_cannot_approve_as_super_admin(self, auth_headers, brands):
        ...
        approve_response = requests.post(
            f"{BASE_URL}/api/users/{user_id}/approve",
            headers=auth_headers,
            json={"role": "super_admin", "brand_scope_ids": brand_ids},
        )

        assert (
            approve_response.status_code == 400
        ), f"Expected 400 when approving as super_admin, got: {approve_response.status_code}"
    ...
    def test_create_user_manually(self, auth_headers, brands):
        ...
        data = response.json()
        assert (
            "generated_password" in data
        ), "Response should contain generated_password"
        assert (
            len(data["generated_password"]) >= 12
        ), "Generated password should be at least 12 characters"
        assert data["user"]["email"] == unique_email
        assert (
            data["user"]["status"] == "active"
        ), "Manually created user should be active immediately"
        ...
        login_response = requests.post(
            f"{BASE_URL}/api/auth/login",
            json={"email": unique_email, "password": generated_pwd},
        )

        assert (
            login_response.status_code == 200
        ), f"Manually created user should be able to login, got: {login_response.status_code}"
        print(
            f"PASS: Manually created user {unique_email} can login with generated password"
        )
    ...
    def test_cannot_create_super_admin_manually(self, auth_headers, brands):
        ...
        ), f"Expected 400 when creating super_admin, got: {response.status_code}"
    ...
    def test_get_all_users_includes_status(self, auth_headers):
        ...
        for user in users:
            ...
            assert user["status"] in [
                "active",
                "pending",
                "rejected",
            ], f"Invalid status: {user['status']}"
        ...
        print(
            f"  Status breakdown: {active_count} active, {pending_count} pending, {rejected_count} rejected"
        )
    ...
    def test_cleanup_test_users(self):
        ...
        for user in test_users:
            delete_response = requests.delete(
                f"{BASE_URL}/api/users/{user['id']}", headers=headers
            )
