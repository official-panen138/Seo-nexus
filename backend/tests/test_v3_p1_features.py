...
class TestExportAssetDomains:
    ...
    def test_export_asset_domains_json(self, auth_headers):
        ...
        response = requests.get(
            f"{V3_API}/export/asset-domains?format=json", headers=auth_headers
        )
    ...
    def test_export_asset_domains_csv(self, auth_headers):
        ...
        response = requests.get(
            f"{V3_API}/export/asset-domains?format=csv", headers=auth_headers
        )
    ...
    def test_export_asset_domains_with_filters(self, auth_headers):
        ...
        response = requests.get(
            f"{V3_API}/export/asset-domains?format=json&status=active",
        ...
        for item in data.get("data", []):
            if "status" in item:
                assert (
                    item["status"] == "active"
                ), f"Filtered item has wrong status: {item.get('status')}"
...
class TestExportNetworkStructure:
    ...
    def test_export_single_network_json(self, auth_headers, test_network_id):
        ...
        response = requests.get(
            f"{V3_API}/export/networks/{test_network_id}?format=json",
            headers=auth_headers,
        )
        ...
        assert (
            "tier_distribution" in data
        ), "Response should have 'tier_distribution' field"
        assert "exported_at" in data, "Response should have 'exported_at' field"
        ...
        print(
            f"PASS: Exported network '{network.get('name')}' with {data['total_entries']} entries"
        )

    def test_export_single_network_csv(self, auth_headers, test_network_id):
        """Export single network with full structure as CSV"""
        response = requests.get(
            f"{V3_API}/export/networks/{test_network_id}?format=csv",
            headers=auth_headers,
        )
    ...
    def test_export_nonexistent_network(self, auth_headers):
        ...
        response = requests.get(
            f"{V3_API}/export/networks/nonexistent-id?format=json", headers=auth_headers
        )
...
class TestExportAllNetworks:
    ...
    def test_export_all_networks_json(self, auth_headers):
        ...
        response = requests.get(
            f"{V3_API}/export/networks?format=json", headers=auth_headers
        )
    ...
    def test_export_all_networks_csv(self, auth_headers):
        ...
        response = requests.get(
            f"{V3_API}/export/networks?format=csv", headers=auth_headers
        )
...
class TestExportActivityLogs:
    ...
    def test_export_activity_logs_json(self, auth_headers):
        ...
        response = requests.get(
            f"{V3_API}/export/activity-logs?format=json", headers=auth_headers
        )
    ...
    def test_export_activity_logs_csv(self, auth_headers):
        ...
        response = requests.get(
            f"{V3_API}/export/activity-logs?format=csv", headers=auth_headers
        )
...
class TestDashboardRefresh:
    ...
    def test_get_dashboard_refresh_setting(self, auth_headers):
        ...
        response = requests.get(
            f"{V3_API}/settings/dashboard-refresh", headers=auth_headers
        )
    ...
    def test_update_dashboard_refresh_setting(self, auth_headers):
        ...
        response = requests.put(
            f"{V3_API}/settings/dashboard-refresh?interval=60", headers=auth_headers
        )
    ...
    def test_update_invalid_refresh_interval(self, auth_headers):
        ...
        response = requests.put(
            f"{V3_API}/settings/dashboard-refresh?interval=45", headers=auth_headers
        )
    ...
    def test_get_dashboard_stats(self, auth_headers):
        ...
        print(
            f"PASS: Got dashboard stats: {data['total_domains']} domains, {data['total_networks']} networks"
        )
...
class TestBulkNodeImport:
    ...
    def test_bulk_import_nodes(self, auth_headers, test_network_id):
        ...
        response = requests.post(
            f"{V3_API}/import/nodes", headers=auth_headers, json=payload
        )
        ...
        print(
            f"PASS: Bulk import - {summary['imported']} imported, {summary['skipped']} skipped, {summary['errors']} errors"
        )
    ...
    def test_bulk_import_with_target_resolution(self, auth_headers, test_network_id):
        ...
        target_response = requests.post(
            f"{V3_API}/import/nodes", headers=auth_headers, json=target_payload
        )
        ...
        response = requests.post(
            f"{V3_API}/import/nodes", headers=auth_headers, json=linking_payload
        )
    ...
    def test_bulk_import_missing_domain_without_create(
        self, auth_headers, test_network_id
    ):
...
class TestAuthenticationRequired:
    ...
    def test_import_nodes_requires_auth(self):
        ...
        response = requests.post(
            f"{V3_API}/import/nodes", json={"network_id": "x", "nodes": []}
        )
        assert response.status_code in [
            401,
            403,
