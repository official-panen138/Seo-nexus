...
class TestSchedulerIntegration:
    ...
    def setup(self):
        ...
        login_response = requests.post(
            ...
            json={"email": "admin152133@example.com", "password": "admin123"},
        )
    ...
    def test_scheduler_status_returns_running(self):
        ...
        response = requests.get(
            f"{BASE_URL}/api/v3/scheduler/reminder-status", headers=self.headers
        )
        ...
        assert (
            scheduler_info.get("job_id") == "optimization_reminder_job"
        ), "Job ID should be optimization_reminder_job"
    ...
    def test_trigger_reminders_manually(self):
        ...
        response = requests.post(
            f"{BASE_URL}/api/v3/scheduler/trigger-reminders", headers=self.headers
        )
        ...
        assert "Reminder job triggered successfully" in data.get(
            "message", ""
        ), "Should confirm successful trigger"
        assert (
            data.get("triggered_by") == "admin152133@example.com"
        ), "Should show triggering user"
        ...
        assert (
            logs_after_count > logs_before_count
        ), f"Expected new log entry. Before: {logs_before_count}, After: {logs_after_count}"
        print(
            f"✓ Execution log count increased from {logs_before_count} to {logs_after_count}"
        )
    ...
    def test_execution_logs_returns_history(self):
        ...
        if logs:
            ...
            if results:
                print(
                    f"✓ Results: total_found={results.get('total_found')}, reminders_sent={results.get('reminders_sent')}, reminders_failed={results.get('reminders_failed')}"
                )
    ...
    def test_execution_logs_filter_by_job_id(self):
        ...
        for log in logs:
            assert (
                log.get("job_id") == "optimization_reminder_job"
            ), f"Expected job_id 'optimization_reminder_job', got '{log.get('job_id')}'"
    ...
    def test_scheduler_endpoints_require_super_admin(self):
        ...
        viewer_login = requests.post(
            f"{BASE_URL}/api/auth/login",
            json={"email": viewer_email, "password": "viewer123"},
        )
        ...
        status_response = requests.get(
            f"{BASE_URL}/api/v3/scheduler/reminder-status", headers=viewer_headers
        )
        assert (
            status_response.status_code == 403
        ), f"Expected 403 for viewer, got {status_response.status_code}"
        print("✓ GET /scheduler/reminder-status returns 403 for viewer")

        # Test trigger endpoint
        trigger_response = requests.post(
            f"{BASE_URL}/api/v3/scheduler/trigger-reminders", headers=viewer_headers
        )
        assert (
            trigger_response.status_code == 403
        ), f"Expected 403 for viewer, got {trigger_response.status_code}"
        print("✓ POST /scheduler/trigger-reminders returns 403 for viewer")

        # Test execution logs endpoint
        logs_response = requests.get(
            f"{BASE_URL}/api/v3/scheduler/execution-logs", headers=viewer_headers
        )
        assert (
            logs_response.status_code == 403
        ), f"Expected 403 for viewer, got {logs_response.status_code}"
    ...
    def test_scheduler_status_fields_structure(self):
        ...
        assert isinstance(
            scheduler.get("interval_hours"), int
        ), "interval_hours should be int"

        # next_run_time should be string (ISO format) or None
        next_run = scheduler.get("next_run_time")
        assert next_run is None or isinstance(
            next_run, str
        ), "next_run_time should be string or None"
    ...
    def test_execution_log_structure(self):
        ...
        assert log.get("status") in [
            "success",
            "failed",
        ], f"Status should be 'success' or 'failed', got '{log.get('status')}'"

        # If success, should have results
        if log.get("status") == "success":
            results = log.get("results", {})
            assert (
                "total_found" in results or results == {}
            ), "Successful log should have results with total_found (or empty results)"
...
class TestSchedulerLifecycle:
    ...
    def test_scheduler_is_initialized_on_startup(self):
        ...
        assert (
            data.get("status") != "not_initialized"
        ), "Scheduler should be initialized on startup"

        scheduler = data.get("scheduler", {})
        assert (
            scheduler.get("running") == True
        ), "Scheduler should be running after server startup"
    ...
    def test_scheduler_job_is_configured_for_24_hours(self):
        ...
        assert (
            scheduler.get("interval_hours") == 24
        ), f"Expected 24-hour interval, got {scheduler.get('interval_hours')}"
