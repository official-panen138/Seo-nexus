...
class TestWeeklyDigestGetSettings:
    ...
    def test_get_digest_settings_success(self, auth_headers):
        ...
        response = requests.get(
            f"{BASE_URL}/api/v3/settings/weekly-digest", headers=auth_headers
        )
        ...
        print(
            f"✓ GET digest settings returned: enabled={data['enabled']}, schedule_day={data['schedule_day']}"
        )
...
class TestWeeklyDigestUpdateSettings:
    ...
    def test_update_unauthorized(self):
        ...
        response = requests.put(
            f"{BASE_URL}/api/v3/settings/weekly-digest", json={"enabled": True}
        )
...
class TestWeeklyDigestPreview:
    ...
    def test_preview_success(self, auth_headers):
        ...
        response = requests.get(
            f"{BASE_URL}/api/v3/settings/weekly-digest/preview", headers=auth_headers
        )
        ...
        print(
            f"✓ Preview returned: subject='{data['subject'][:50]}...', total_issues={data['total_issues']}"
        )
    ...
    def test_preview_expiring_domains_structure(self, auth_headers):
        ...
        if "expiring_domains" in data and data["expiring_domains"]:
            ...
            print(
                f"✓ expiring_domains has urgency groups: critical={len(expiring.get('critical', []))}, high={len(expiring.get('high', []))}, medium={len(expiring.get('medium', []))}"
            )
        else:
...
class TestWeeklyDigestSend:
    ...
    def test_send_returns_error_without_api_key(self, auth_headers):
        ...
        response = requests.post(
            f"{BASE_URL}/api/v3/settings/weekly-digest/send", headers=auth_headers
        )
        ...
        assert response.status_code in [
            ...
            400,
            520,
        ], f"Expected 500/400/520, got {response.status_code}: {response.text}"
