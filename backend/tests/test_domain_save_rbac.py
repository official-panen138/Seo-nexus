...
class TestAuthentication:
    ...
    def test_manager_login_success(self):
        ...
        assert data["user"]["brand_scope_ids"] is not None, "Manager should have brand_scope_ids"
        assert MANAGER_BRAND_ID in data["user"]["brand_scope_ids"], "Manager should have Panen138 brand access"
    ...
    def test_super_admin_login_success(self):
        ...
        assert data["user"]["brand_scope_ids"] is None, "Super admin should have NULL brand_scope_ids (full access)"
...
class TestNetworkRBAC:
    ...
    def test_manager_sees_only_their_brand_networks(self, manager_token):
        ...
        for network in networks:
            assert network.get("brand_id") == MANAGER_BRAND_ID, \
    ...
    def test_super_admin_sees_all_networks(self, admin_token):
        ...
        assert len(networks) >= 3, "Super admin should see at least as many networks as manager"
    ...
    def test_manager_vs_admin_network_count(self, manager_token, admin_token):
        ...
        assert admin_count >= manager_count, "Super admin should see at least as many networks as manager"
...
class TestDomainRBAC:
    ...
    def test_manager_sees_only_their_brand_domains(self, manager_token):
        ...
        for domain in domains:
            assert domain.get("brand_id") == MANAGER_BRAND_ID, \
...
class TestDomainSave:
    ...
    def test_manager_can_create_domain_for_their_brand(self, manager_token):
        ...
        assert response.status_code == 200, f"Manager domain creation failed: {response.text}"
        data = response.json()
        print(f"✓ Manager created domain: {domain_name}")
        
        # Cleanup
        admin_token = requests.post(f"{BASE_URL}/api/auth/login", json=SUPER_ADMIN_CREDS).json().get("access_token")
        requests.delete(
            f"{BASE_URL}/api/v3/asset-domains/{data['id']}",
            headers={"Authorization": f"Bearer {admin_token}"}
        )
...
class TestDomainListFiltering:
    ...
    def test_domain_list_filtered_by_brand_id(self, admin_token):
        ...
        print(f"✓ Domain list correctly filtered by brand_id ({data['meta']['total']} domains)")
...
class TestManagerDashboard:
    ...
    def test_manager_brands_list_restricted(self, manager_token):
        ...
        for brand in brands:
            assert brand["id"] == MANAGER_BRAND_ID, \
