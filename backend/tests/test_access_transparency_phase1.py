...
class TestNetworkListAccessSummary:
    ...
    def test_networks_list_includes_visibility_mode(self, api_client):
        ...
        if test_network:
            assert (
                "visibility_mode" in test_network
            ), "Network should have 'visibility_mode' field"
            assert test_network["visibility_mode"] in [
                "brand_based",
                "restricted",
                "public",
            ], f"visibility_mode should be valid value, got: {test_network['visibility_mode']}"
            print(
                f"PASS: Network has visibility_mode: {test_network['visibility_mode']}"
            )
        else:
    ...
    def test_networks_list_includes_access_summary_cache(self, api_client):
        ...
        if test_network and test_network.get("visibility_mode") == "restricted":
            ...
            if cache is not None:
                assert (
                    "count" in cache
                ), "access_summary_cache should have 'count' field"
                assert (
                    "names" in cache
                ), "access_summary_cache should have 'names' field"
                assert isinstance(cache["count"], int), "count should be integer"
                assert isinstance(cache["names"], list), "names should be list"
                print(
                    f"PASS: access_summary_cache present: count={cache['count']}, names={cache['names']}"
                )
            else:
                print(
                    f"PASS: access_summary_cache is None (acceptable for empty restricted)"
                )
        else:
            print(
                f"PASS: Network list retrieved (test network visibility_mode: {test_network.get('visibility_mode') if test_network else 'not found'})"
            )
...
class TestAccessSummaryCachePopulation:
    ...
    def test_save_restricted_populates_access_summary_cache(
        self, api_client, current_user_id
    ):
        ...
        assert (
            "access_summary_cache" in result
        ), "Response should include access_summary_cache"
        ...
        print(
            f"PASS: access_summary_cache populated on save: count={cache['count']}, names={cache['names']}"
        )
        ...
        test_network = next(
            (n for n in networks if n.get("id") == TEST_NETWORK_ID), None
        )

        if test_network:
            net_cache = test_network.get("access_summary_cache", {})
            assert (
                net_cache.get("count") == 1
            ), "Network list should reflect updated cache"
    ...
    def test_save_brand_based_clears_access_summary_cache(self, api_client):
        ...
        assert (
            cache.get("count", 0) == 0 or len(cache.get("names", [])) == 0
...
class TestAuditLogCreation:
    ...
    def test_save_access_creates_audit_log(self, api_client, current_user_id):
        ...
        logs_response = api_client.get(
            f"{BASE_URL}/api/v3/networks/{TEST_NETWORK_ID}/access-audit-logs"
        )
        assert (
        ...
        assert (
            recent_log.get("event_type") == "NETWORK_ACCESS_CHANGED"
        ...
        print(
            f"PASS: Audit log created: event_type={recent_log['event_type']}, new_mode={recent_log['new_mode']}"
        )
    ...
    def test_audit_log_tracks_added_users(self, api_client, current_user_id):
        ...
        for log in logs:
            if log.get("added_user_ids"):
                assert (
                    "added_user_names" in log
                ), "Should have added_user_names when added_user_ids exists"
                print(
                    f"PASS: Audit log tracks added users: {log.get('added_user_names')}"
                )
                return
...
class TestLastUpdatedInfo:
    ...
    def test_access_control_includes_last_updated_at(self, api_client, current_user_id):
        ...
        data = response.json()
        assert (
            "access_updated_at" in data
        ), "Response should include 'access_updated_at'"
        assert (
            data["access_updated_at"] is not None
        ), "access_updated_at should not be None"

        # Validate it's a valid ISO timestamp
        try:
            datetime.fromisoformat(data["access_updated_at"].replace("Z", "+00:00"))
            print(
                f"PASS: access_updated_at is valid timestamp: {data['access_updated_at']}"
            )
        except ValueError:
            pytest.fail(
                f"access_updated_at is not valid ISO timestamp: {data['access_updated_at']}"
            )
    ...
    def test_access_control_includes_last_updated_by(self, api_client, current_user_id):
        ...
        data = response.json()
        assert (
            "access_updated_by" in data
        ), "Response should include 'access_updated_by'"
        ...
        print(
            f"PASS: access_updated_by: {updated_by.get('email')}, name: {updated_by.get('name')}"
        )
...
class TestAccessAuditLogsEndpoint:
    ...
    def test_audit_logs_structure(self, api_client, current_user_id):
        ...
        if logs:
            ...
            assert "user_id" in log.get(
                "changed_by", {}
            ), "changed_by should have user_id"
