...
class TestPhase3ReminderConfig:
    ...
    def test_get_global_reminder_config(self, auth_headers):
        ...
        response = requests.get(
            f"{BASE_URL}/api/v3/settings/reminder-config", headers=auth_headers
        )
        ...
        print(
            f"Global reminder config: enabled={data['enabled']}, interval={data['interval_days']} days"
        )
    ...
    def test_update_global_reminder_config(self, auth_headers):
        ...
        print(
            f"Updated global interval from {original_interval} to {new_interval}, then restored"
        )
    ...
    def test_get_network_reminder_config(self, auth_headers):
        ...
        override_exists = (
            bool(data.get("network_override"))
            if "network_override" in data
            else data.get("has_override", False)
        )
        print(
            f"Network reminder config: effective={data['effective_interval_days']}, has_override={override_exists}"
        )
    ...
    def test_update_network_reminder_config_set_override(self, auth_headers):
        ...
        has_override = (
            bool(verify_data.get("network_override"))
            if "network_override" in verify_data
            else verify_data.get("has_override", False)
        )
        assert has_override == True, f"Override should be set, got: {verify_data}"
        assert (
            verify_data.get("effective_interval_days") == 5
        ), "Effective interval should be 5"
    ...
    def test_update_network_reminder_config_clear_override(self, auth_headers):
        ...
        global_interval = (
            verify_data.get("global_default", {}).get("interval_days")
            if "global_default" in verify_data
            else verify_data.get("global_interval_days")
        )
    ...
    def test_get_optimization_reminders(self, auth_headers):
        ...
        response = requests.get(
            f"{BASE_URL}/api/v3/optimization-reminders", headers=auth_headers
        )
        assert response.status_code == 200, f"Failed: {response.text}"
...
class TestPhase2ComplaintAutoTagging:
    ...
    def test_create_complaint_auto_tags_network_users(self, auth_headers):
        ...
        assert (
            complaint_response.status_code == 200
        ), f"Failed to create complaint: {complaint_response.text}"
        ...
        if visibility_mode == "restricted" and network_allowed_users:
            ...
            for user_id in network_allowed_users:
                assert (
                    user_id in complaint["responsible_user_ids"]
                ), f"Network user {user_id} should be auto-assigned to complaint"
            print(
                f"Auto-tagging verified: {len(complaint['responsible_user_ids'])} users assigned from network"
            )
        else:
            print(
                f"Network mode is '{visibility_mode}' - auto-tagging only applies to 'restricted' mode with allowed users"
            )
...
class TestPhase1Verification:
    ...
    def test_networks_list_includes_visibility_mode(self, auth_headers):
        ...
        response = requests.get(f"{BASE_URL}/api/v3/networks", headers=auth_headers)
        ...
        if networks:
            ...
            network = networks[0]
            assert (
                "visibility_mode" in network
                or network.get("visibility_mode") is not None
            ), "visibility_mode should be present (defaults to brand_based)"
            print(
                f"First network visibility_mode: {network.get('visibility_mode', 'brand_based')}"
            )
