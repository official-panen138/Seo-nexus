    ...
    def test_change_log_has_snapshots(self, headers):
        ...
        if len(data) > 0:
            ...
            if log.get("action_type") in ["update_node", "relink_node", "change_role"]:
                assert (
                    log["before_snapshot"] is not None
                ), "Update should have before_snapshot"
                assert (
                    log["after_snapshot"] is not None
                ), "Update should have after_snapshot"
    ...
    def test_change_log_has_entry_id_for_graph_highlight(self, headers):
        ...
        if len(data) > 0:
            ...
            if log.get("after_snapshot"):
                assert (
                    "id" in log["after_snapshot"]
                ), "after_snapshot should have entry id"
...
class TestMarkNotificationsReadAPI:
    ...
    def test_mark_all_notifications_read(self, headers):
        ...
        data = response.json()
        assert (
            "count" in data or "message" in data
        ), "Response should have count or message"
...
class TestChangeHistoryIntegration:
    ...
    def test_notification_types_are_valid(self, headers):
        ...
        for notif in data:
            ...
            assert (
                notif_type in valid_notif_types
