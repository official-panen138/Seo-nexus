...
class TestDomainMonitoringTelegramAPI:
    ...
    def test_post_telegram_monitoring_test_endpoint_exists(self, auth_headers):
        ...
        response = requests.post(
            f"{BASE_URL}/api/v3/settings/telegram-monitoring/test",
            headers=auth_headers
        )
        ...
        if response.status_code == 500:
            ...
            assert "configuration" in data["detail"].lower() or "token" in data["detail"].lower()
...
class TestImpactScoreCalculation:
    ...
    def test_impact_score_severity_medium(self):
        ...
        assert score["severity"] in ["MEDIUM", "HIGH"]  # Could be HIGH due to reaches_money_site
