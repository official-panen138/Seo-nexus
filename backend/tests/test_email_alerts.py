...
if not BASE_URL:
    BASE_URL = "https://seo-alerts-overhaul.preview.emergentagent.com"
...
class TestEmailAlertsAPI:
    ...
    def test_get_email_alert_settings_returns_200(self, headers):
        ...
        response = requests.get(f"{BASE_URL}/api/v3/settings/email-alerts", headers=headers)
    ...
    def test_get_email_alert_settings_structure(self, headers):
        ...
        assert "include_network_managers" in data, "Missing 'include_network_managers' field"
        ...
        assert isinstance(data["global_admin_emails"], list), "global_admin_emails should be list"
        assert isinstance(data["include_network_managers"], bool), "include_network_managers should be boolean"
    ...
    def test_update_email_alert_settings_enabled(self, headers):
        ...
        get_response = requests.get(f"{BASE_URL}/api/v3/settings/email-alerts", headers=headers)
    ...
    def test_update_email_alert_settings_severity_threshold(self, headers):
        ...
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
        data = response.json()
        assert data["severity_threshold"] == "critical", f"Expected 'critical', got {data['severity_threshold']}"
    ...
    def test_update_email_alert_settings_admin_emails(self, headers):
        ...
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
        data = response.json()
        assert set(data["global_admin_emails"]) == set(test_emails), f"Expected {test_emails}, got {data['global_admin_emails']}"
    ...
    def test_email_alerts_test_endpoint_exists(self, headers):
        ...
        if response.status_code == 500:
            ...
            print(f"✓ Test endpoint exists, returns proper error (no API key): {data.get('detail')}")
        else:
