...
class TestAccessSummaryPanelAPI:
    ...
    def test_networks_list_returns_open_complaints_count(self):
        ...
        for network in networks:
            assert "open_complaints_count" in network, f"Missing open_complaints_count in network {network.get('id')}"
            assert isinstance(network["open_complaints_count"], int), "open_complaints_count should be integer"
            print(f"Network '{network.get('name')}': open_complaints_count = {network['open_complaints_count']}")
    ...
    def test_networks_list_returns_last_optimization_at(self):
        ...
        for network in networks:
            assert "last_optimization_at" in network, f"Missing last_optimization_at in network {network.get('id')}"
            # Can be null if no optimizations exist
            if network["last_optimization_at"] is not None:
                assert isinstance(network["last_optimization_at"], str), "last_optimization_at should be string (ISO date)"
            print(f"Network '{network.get('name')}': last_optimization_at = {network['last_optimization_at']}")
    ...
    def test_network_detail_returns_open_complaints_count(self):
        ...
        list_response = requests.get(f"{BASE_URL}/api/v3/networks", headers=self.headers)
        ...
        response = requests.get(f"{BASE_URL}/api/v3/networks/{network_id}", headers=self.headers)
        
        assert response.status_code == 200, f"API failed: {response.text}"
        network = response.json()
        
        assert "open_complaints_count" in network, "Missing open_complaints_count in network detail"
        assert isinstance(network["open_complaints_count"], int), "open_complaints_count should be integer"
        print(f"Network detail '{network.get('name')}': open_complaints_count = {network['open_complaints_count']}")
    ...
    def test_network_detail_returns_last_optimization_at(self):
        ...
        assert "last_optimization_at" in network, "Missing last_optimization_at in network detail"
        # Can be null if no optimizations exist
        if network["last_optimization_at"] is not None:
            assert isinstance(network["last_optimization_at"], str), "last_optimization_at should be string (ISO date)"
        print(f"Network detail '{network.get('name')}': last_optimization_at = {network['last_optimization_at']}")
    ...
    def test_network_with_open_complaint_shows_count(self):
        ...
        if response.status_code == 200:
            network = response.json()
            print(f"Network '{network.get('name')}': open_complaints_count = {network.get('open_complaints_count')}")
            assert network.get("open_complaints_count", 0) >= 1, "Expected at least 1 open complaint"
        elif response.status_code == 404:
            pytest.skip("Network 76e067db-60ba-4e3b-a949-b3229dc1c652 not found - skipping")
        else:
    ...
    def test_networks_list_includes_manager_summary_cache(self):
        ...
        for network in networks:
            ...
            if network.get("visibility_mode") == "restricted":
                ...
                if manager_cache:
                    assert "count" in manager_cache, "manager_summary_cache should have count"
                    assert "names" in manager_cache, "manager_summary_cache should have names"
                    print(f"Network '{network.get('name')}' (restricted): {manager_cache['count']} managers: {manager_cache['names']}")
            else:
                print(f"Network '{network.get('name')}' (brand_based): visibility_mode = {network.get('visibility_mode')}")
    ...
    def test_networks_list_includes_visibility_mode(self):
        ...
        for network in networks:
            ...
            if visibility_mode is not None:
                assert visibility_mode in ["brand_based", "restricted"], f"Invalid visibility_mode: {visibility_mode}"
            print(f"Network '{network.get('name')}': visibility_mode = {visibility_mode or 'brand_based (default)'}")
...
class TestNetworkDetailHeaderAccessSummary:
    ...
    def test_network_detail_has_all_access_summary_fields(self):
        ...
        for field in required_fields:
            assert field in network, f"Missing field '{field}' in network detail response"
