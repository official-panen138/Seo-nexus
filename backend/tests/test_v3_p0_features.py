...
class TestNetworkBrandRequirement:
    ...
    def test_create_network_with_valid_brand(self, super_admin_token, existing_brand_id):
    ...
    def test_network_list_shows_brand(self, super_admin_token):
        ...
        for network in data:
            ...
            print(f"  Network: {network['name']} - Brand: {network.get('brand_name', 'N/A')}")
...
class TestPathLevelSeoNodes:
    ...
    def test_structure_entry_has_optimized_path(self, super_admin_token, test_network_id):
        ...
        print(f"✓ Found {len(path_entries)} entries with optimized_path out of {len(entries)} total")
    ...
    def test_structure_entry_has_target_entry_id(self, super_admin_token, test_network_id):
        ...
        for entry in entries:
            ...
            has_target = entry.get("target_entry_id") or entry.get("target_asset_domain_id")
            if entry.get("domain_role") != "main":
                print(f"  Entry: {entry.get('domain_name')} -> Target: {has_target or 'None (orphan)'}")
...
class TestTierCalculation:
    ...
    def test_entries_have_calculated_tier(self, super_admin_token, test_network_id):
        ...
        for entry in entries:
            ...
            print(f"  {entry['domain_name']}: {entry['tier_label']} (tier {entry['calculated_tier']})")
    ...
    def test_main_domain_is_tier_zero(self, super_admin_token, test_network_id):
        ...
        for main in main_entries:
            ...
            assert "LP/Money Site" in main["tier_label"] or "Money" in main["tier_label"]
            print(f"✓ Main domain {main['domain_name']} is tier 0: {main['tier_label']}")
...
class TestRegistrarInDomainForm:
    ...
    def test_create_asset_domain_with_registrar(self, super_admin_token, existing_brand_id, existing_registrar_id):
        ...
        print(f"✓ Created domain {test_domain} with registrar: {data.get('registrar_name')}")
