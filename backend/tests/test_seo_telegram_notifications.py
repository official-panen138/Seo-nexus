...
class TestSeoTelegramNotifications:
    ...
    def test_get_telegram_seo_returns_topic_routing_fields(self, headers):
        ...
        response = requests.get(f"{BASE_URL}/api/v3/settings/telegram-seo", headers=headers)
        ...
        required_fields = [
            "configured", "enabled", "enable_topic_routing",
            "seo_change_topic_id", "seo_optimization_topic_id",
            "seo_complaint_topic_id", "seo_reminder_topic_id"
        ]
    ...
    def test_get_telegram_seo_returns_correct_types(self, headers):
        ...
        assert isinstance(data["enable_topic_routing"], bool), "enable_topic_routing should be boolean"
        assert isinstance(data["configured"], bool), "configured should be boolean"
        
        # topic_ids can be None or strings
        topic_fields = ["seo_change_topic_id", "seo_optimization_topic_id", 
                        "seo_complaint_topic_id", "seo_reminder_topic_id"]
        for field in topic_fields:
            value = data[field]
            assert value is None or isinstance(value, (str, int)), f"{field} should be None, str, or int"
    ...
    def test_update_telegram_seo_with_topic_routing(self, headers):
        ...
        response = requests.put(f"{BASE_URL}/api/v3/settings/telegram-seo", 
        ...
        # Verify by GET
        get_response = requests.get(f"{BASE_URL}/api/v3/settings/telegram-seo", headers=headers)
        assert get_response.status_code == 200
        
        data = get_response.json()
        assert data["enable_topic_routing"] == True, "enable_topic_routing should be True"
        assert data["seo_change_topic_id"] == "123", "seo_change_topic_id should be '123'"
        assert data["seo_optimization_topic_id"] == "124", "seo_optimization_topic_id should be '124'"
        assert data["seo_complaint_topic_id"] == "125", "seo_complaint_topic_id should be '125'"
        assert data["seo_reminder_topic_id"] == "126", "seo_reminder_topic_id should be '126'"
    ...
    def test_update_telegram_seo_disable_topic_routing(self, headers):
        ...
        data = get_response.json()
        assert data["enable_topic_routing"] == False, "enable_topic_routing should be False"
    ...
    def test_update_telegram_seo_preserves_bot_token(self, headers):
        ...
        get_response1 = requests.get(f"{BASE_URL}/api/v3/settings/telegram-seo", headers=headers)
        original_data = get_response1.json()
        
        # Update only topic routing (no bot_token)
        update_data = {
            "enable_topic_routing": True,
            "seo_change_topic_id": "999"
        }
        ...
        get_response2 = requests.get(f"{BASE_URL}/api/v3/settings/telegram-seo", headers=headers)
        new_data = get_response2.json()
        
        # If there was a bot_token before, it should still be there
        if original_data.get("bot_token"):
            assert new_data.get("bot_token") == original_data.get("bot_token"), "bot_token should be preserved"
    ...
    def test_get_reminder_config_returns_correct_structure(self, headers):
        ...
        response = requests.get(f"{BASE_URL}/api/v3/settings/reminder-config", headers=headers)
        ...
        assert "interval_days" in data or "default_interval_days" in data, "Missing interval_days field"
    ...
    def test_update_reminder_config_interval(self, headers):
        ...
        response = requests.put(f"{BASE_URL}/api/v3/settings/reminder-config", 
                               json=update_data, headers=headers)
        assert response.status_code == 200, f"PUT reminder-config failed: {response.text}"
        
        result = response.json()
        print(f"PUT /api/v3/settings/reminder-config response: {result}")
        
        # Verify by GET
        get_response = requests.get(f"{BASE_URL}/api/v3/settings/reminder-config", headers=headers)
        assert get_response.status_code == 200
        
        data = get_response.json()
        assert data.get("interval_days") == 3 or data.get("default_interval_days") == 3, \
    ...
    def test_update_reminder_config_invalid_interval_too_low(self, headers):
        ...
        response = requests.put(f"{BASE_URL}/api/v3/settings/reminder-config", 
                               json=update_data, headers=headers)
        assert response.status_code == 400, f"Expected 400 for interval=0: {response.status_code}"
    ...
    def test_update_reminder_config_invalid_interval_too_high(self, headers):
        ...
        response = requests.put(f"{BASE_URL}/api/v3/settings/reminder-config", 
                               json=update_data, headers=headers)
        assert response.status_code == 400, f"Expected 400 for interval=31: {response.status_code}"
    ...
    def test_run_reminder_check_returns_stats(self, headers):
        ...
        response = requests.post(f"{BASE_URL}/api/v3/settings/reminder-config/run", headers=headers)
        assert response.status_code == 200, f"POST reminder-config/run failed: {response.text}"
    ...
    def test_telegram_seo_requires_super_admin_for_update(self):
        ...
        response = requests.put(f"{BASE_URL}/api/v3/settings/telegram-seo", json={
            "enable_topic_routing": True
        })
    ...
    def test_reminder_config_requires_super_admin(self):
        ...
        response = requests.put(f"{BASE_URL}/api/v3/settings/reminder-config", json={
            "enabled": True,
            "interval_days": 5
        })
        assert response.status_code in [401, 403], f"Expected 401/403 without auth: {response.status_code}"
...
class TestNotificationTopicRouting:
    ...
    def test_telegram_test_endpoint_exists(self, headers):
        ...
        response = requests.post(f"{BASE_URL}/api/v3/settings/telegram-seo/test", headers=headers)
        ...
        if response.status_code == 200:
            ...
            print(f"Telegram test endpoint exists but returned: {response.status_code} (may need configuration)")
...
class TestSettingsPageAPI:
    ...
    def test_frontend_can_load_seo_telegram_settings(self, headers):
        ...
        frontend_fields = [
            "bot_token", "chat_id", "enabled", "enable_topic_routing",
            "seo_change_topic_id", "seo_optimization_topic_id",
            "seo_complaint_topic_id", "seo_reminder_topic_id"
        ]
    ...
    def test_frontend_can_toggle_topic_routing(self, headers):
        ...
        # Disable
        response = requests.put(f"{BASE_URL}/api/v3/settings/telegram-seo", 
                               json={"enable_topic_routing": False}, headers=headers)
        assert response.status_code == 200
        
        # Verify disabled
        get_resp = requests.get(f"{BASE_URL}/api/v3/settings/telegram-seo", headers=headers)
        assert get_resp.json()["enable_topic_routing"] == False
        
        # Enable
        response = requests.put(f"{BASE_URL}/api/v3/settings/telegram-seo", 
                               json={"enable_topic_routing": True}, headers=headers)
        assert response.status_code == 200
