...
class TestReminderConfigAPI:
    ...
    def test_get_reminder_config_has_global_default(self):
        ...
        assert "interval_days" in data.get("global_default", {}), "global_default must have interval_days"
    ...
    def test_get_reminder_config_has_effective_interval(self):
        ...
        data = response.json()
        assert "effective_interval_days" in data, "Response must include effective_interval_days"
        assert isinstance(data["effective_interval_days"], int), "effective_interval_days must be int"
    ...
    def test_put_reminder_config_with_custom_interval_returns_200(self):
        ...
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
        data = response.json()
        assert "interval_days" in data or "message" in data, "Response should include interval_days or message"
    ...
    def test_put_reminder_config_reset_to_global(self):
        ...
        assert data.get("network_override") in [None, {}], "network_override should be empty after reset"
...
class TestConflictsAPI:
    ...
    def test_get_conflicts_structure_if_any(self):
        ...
        if len(conflicts) > 0:
            ...
            assert "description" in conflict or "message" in conflict, "Conflict must have description or message"
        else:
            print("No conflicts detected - this is expected if networks are configured correctly")
