...
class TestChangeNoteMainNodeSwitch:
    ...
    def test_main_node_validation_no_target_entry_id(self):
        ...
        assert (
            networks_resp.status_code == 200
        ), f"Failed to get networks: {networks_resp.text}"
        ...
        network_resp = requests.get(
            f"{BASE_URL}/api/v3/networks/{network['id']}", headers=self.headers
        )
        ...
        supporting_entry = next(
            (e for e in entries if e["domain_role"] != "main"), None
        )
        ...
        ), f"Should reject main with target_entry_id: {create_resp.text}"
        error = create_resp.json()
        assert (
            "target" in error.get("detail", "").lower()
            or "main" in error.get("detail", "").lower()
    ...
    def test_main_node_validation_rejects_canonical_status(self):
        ...
        assert (
            create_resp.status_code == 400
        ), f"Should reject main with canonical: {create_resp.text}"
        error = create_resp.json()
        assert (
            "primary" in error.get("detail", "").lower()
            or "main" in error.get("detail", "").lower()
    ...
    def test_main_node_validation_rejects_redirect_status(self):
        ...
        assert (
            create_resp.status_code == 400
        ), f"Should reject main with redirect: {create_resp.text}"
    ...
    def test_switch_main_target_api_exists(self):
        ...
        assert resp.status_code in [
            400,
            404,
        ], f"Switch endpoint should exist: {resp.status_code} {resp.text}"
    ...
    def test_switch_main_target_validates_change_note(self):
        ...
        ), f"Should require change_note: {resp.status_code}"
    ...
    def test_switch_main_target_validates_min_chars(self):
        ...
        assert (
            resp.status_code == 422
        ), f"Should reject short change_note: {resp.status_code}"
    ...
    def test_switch_main_target_success_verifies_tier_recalculation(self):
        ...
        networks = networks_resp.json()
        test_network = next(
            (n for n in networks if "TESTER" in n.get("name", "")), None
        )
        if not test_network:
            pytest.skip("No TESTER network available")

        network_detail = requests.get(
            f"{BASE_URL}/api/v3/networks/{test_network['id']}", headers=self.headers
        ).json()
        ...
        assert (
            new_main["domain_role"] == "main"
        ), f"New main should have main role: {new_main['domain_role']}"
        assert (
            new_main["domain_status"] == "primary"
        ), f"New main should have primary status: {new_main['domain_status']}"
        assert (
            new_main.get("target_entry_id") is None
        ), f"New main should have no target"
        ...
        assert (
            old_main["domain_role"] == "supporting"
        ), f"Old main should be supporting: {old_main['domain_role']}"
        assert (
            old_main["domain_status"] == "canonical"
        ), f"Old main should be canonical: {old_main['domain_status']}"
        assert (
            old_main.get("target_entry_id") == new_main_id
        ), f"Old main should target new main"

        # Verify tier recalculation - new main should be tier 0
        assert (
            new_main.get("calculated_tier") == 0
        ), f"New main should be tier 0: {new_main.get('calculated_tier')}"
    ...
    def test_change_note_max_length_2000(self):
        ...
        ), f"Should accept 2000 char note: {resp.status_code}"
    ...
    def test_change_note_rejects_over_2000(self):
        ...
        assert (
            resp.status_code == 422
        ), f"Should reject > 2000 char note: {resp.status_code}"
...
class TestMainNodeStatuses:
    ...
    def test_models_define_main_node_allowed_statuses(self):
        ...
        assert (
            len(MAIN_NODE_ALLOWED_STATUSES) == 1
        ), "Main nodes should only allow PRIMARY"
