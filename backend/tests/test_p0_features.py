...
class TestAssetDomainSeoNetworksField:
    ...
    def test_seo_networks_contains_network_info(self, auth_headers):
        ...
        if domain_with_networks:
            ...
            assert (
                "network_name" in network_info
            ), "Network info should have 'network_name'"
            assert "role" in network_info, "Network info should have 'role'"
            assert network_info["role"] in [
                "main",
                "supporting",
            ], f"Role should be main/supporting, got {network_info['role']}"
            print(
                f"✓ Domain '{domain_with_networks['domain_name']}' has {len(domain_with_networks['seo_networks'])} networks"
            )
        else:
    ...
    def test_unused_domains_have_empty_seo_networks(self, auth_headers):
        ...
        if domain_without_networks:
            assert (
                domain_without_networks["seo_networks"] == []
            ), "Unused domain should have empty seo_networks"
            print(
                f"✓ Unused domain '{domain_without_networks['domain_name']}' has empty seo_networks"
            )
        else:
...
class TestBrandScopingForSearch:
    ...
    def test_search_requires_auth(self):
        ...
        response = requests.get(
            f"{BASE_URL}/api/v3/networks/search", params={"query": "test"}
        )
