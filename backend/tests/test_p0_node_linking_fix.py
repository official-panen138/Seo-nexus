...
class TestV3NetworksAPINodeLabel:
    ...
    def test_network_entries_have_tier_info(self, api_client):
        ...
        for entry in data["entries"]:
            assert (
                "calculated_tier" in entry
            ), f"Missing calculated_tier in entry {entry['id']}"
            assert "tier_label" in entry, f"Missing tier_label in entry {entry['id']}"
            assert (
                entry["calculated_tier"] is not None
            ), "calculated_tier should not be None"
            print(
                f"{entry['node_label']}: {entry['tier_label']} (tier {entry['calculated_tier']})"
            )
...
class TestAvailableTargetsAPI:
    ...
    def test_available_targets_returns_nodes(self, api_client):
        ...
        for target in data:
            ...
            print(
                f"Available target: {target['node_label']} (id: {target['id']}, role: {target['domain_role']})"
            )
    ...
    def test_available_targets_excludes_self(self, api_client):
        ...
        assert any(
            t["id"] == MAIN_ENTRY_ID for t in data
        ), "Main entry should be available"
    ...
    def test_available_targets_has_correct_structure(self, api_client):
        ...
        for target in data:
            ...
            print(
                f"Target dropdown: value={target['id'][:8]}..., label={target['node_label']}"
            )
...
class TestTierCalculation:
    ...
    def test_main_node_is_tier_0(self, api_client):
        ...
        for main in main_entries:
            assert (
                main["calculated_tier"] == 0
            ), f"Main node should be tier 0, got {main['calculated_tier']}"
            assert (
                main["tier_label"] == "LP/Money Site"
            ), f"Main should be LP/Money Site, got {main['tier_label']}"
            print(
                f"Main node {main['node_label']}: tier={main['calculated_tier']} ({main['tier_label']})"
            )
    ...
    def test_supporting_node_is_higher_tier(self, api_client):
        ...
        supporting_entries = [
            e for e in data["entries"] if e["domain_role"] == "supporting"
        ]

        for entry in supporting_entries:
            # Supporting nodes should have tier >= 1
            assert (
                entry["calculated_tier"] >= 1
            ), f"Supporting should be tier >= 1, got {entry['calculated_tier']}"

            # If it has a target, it should be calculated from the target
            if entry.get("target_entry_id"):
                print(
                    f"Supporting {entry['node_label']}: tier={entry['calculated_tier']} targets={entry.get('target_domain_name')}"
                )
            else:
...
class TestTargetNodeRelationship:
    ...
    def test_supporting_node_has_target(self, api_client):
        ...
        supporting = next(
            (e for e in data["entries"] if e["id"] == SUPPORTING_ENTRY_ID), None
        )
        ...
        assert (
            supporting["target_entry_id"] == MAIN_ENTRY_ID
        ), "Supporting should target main entry"
        assert (
            supporting["target_domain_name"] == "moneysite.com"
        ), "Target domain name should be moneysite.com"

        print(
            f"Supporting node targets: {supporting['target_domain_name']} (entry_id: {supporting['target_entry_id'][:8]}...)"
        )
...
class TestGraphVisualizationLinks:
    ...
    def test_entries_have_link_data(self, api_client):
        ...
        link = links[0]
        assert (
            link["source"] == SUPPORTING_ENTRY_ID
        ), "Source should be supporting entry"
        assert link["target"] == MAIN_ENTRY_ID, "Target should be main entry"

        print(
            f"Graph link: {entry_map[link['source']]['node_label']} -> {entry_map[link['target']]['node_label']}"
        )
    ...
    def test_structure_entry_accepts_empty_path(self, api_client):
        ...
        for entry in data["entries"]:
            ...
            if path:
                assert (
                    path in node_label
                ), f"node_label should include path: {node_label}"
            else:
                assert (
                    entry["domain_name"] == node_label
                ), f"node_label should be domain_name when no path"

            print(
                f"Path normalization: {entry['domain_name']} + '{path or ''}' = {node_label}"
            )
...
class TestUpdateStructureEntry:
    ...
    def test_update_target_entry_id(self, api_client):
        ...
        assert (
            updated["target_entry_id"] == MAIN_ENTRY_ID
        ), "target_entry_id should be updated"
    ...
    def test_cannot_target_self(self, api_client):
        ...
        ), f"Should reject self-target: {response.text}"
        assert (
            "cannot target itself" in response.text.lower()
            or "self" in response.text.lower()
        )
