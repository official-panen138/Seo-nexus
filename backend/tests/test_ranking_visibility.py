...
class TestRankingVisibilityAPI:
    ...
    def test_networks_response_includes_ranking_fields(self):
        ...
        for network in networks:
            ...
            if network["best_ranking_position"] is not None:
                ...
                assert (
                    1 <= network["best_ranking_position"] <= 100
    ...
    def test_project_a_has_ranking_status(self):
        ...
        if project_a:
            assert (
                project_a["ranking_status"] == "ranking"
            ), f"Expected 'ranking' status for Project A, got: {project_a['ranking_status']}"
            assert (
                project_a["ranking_nodes_count"] >= 1
            ), f"Expected at least 1 ranking node for Project A, got: {project_a['ranking_nodes_count']}"
            assert (
                project_a["best_ranking_position"] is not None
            ), "Expected best_ranking_position for Project A"
            print(
                f"✓ Project A has ranking_status='ranking', nodes={project_a['ranking_nodes_count']}, best_pos=#{project_a['best_ranking_position']}"
            )
        else:
    ...
    def test_filter_by_ranking_status_ranking(self):
        ...
        response = requests.get(
            f"{BASE_URL}/api/v3/networks?ranking_status=ranking", headers=self.headers
        )
        assert response.status_code == 200
        ...
        print(
            f"✓ Filter ranking_status=ranking returned {len(networks)} networks (all with status='ranking')"
        )

    def test_filter_by_ranking_status_tracking(self):
        """Test filter ranking_status=tracking returns only tracking networks"""
        response = requests.get(
            f"{BASE_URL}/api/v3/networks?ranking_status=tracking", headers=self.headers
        )
        assert response.status_code == 200
        ...
        for network in networks:
            assert (
                network["ranking_status"] == "tracking"
    ...
    def test_filter_by_ranking_status_none(self):
        ...
        response = requests.get(
            f"{BASE_URL}/api/v3/networks?ranking_status=none", headers=self.headers
        )
        assert response.status_code == 200
        ...
        for network in networks:
            assert (
                network["ranking_status"] == "none"
            ), f"Network {network['name']} should not be in none filter results"

        print(
            f"✓ Filter ranking_status=none returned {len(networks)} networks (all with status='none')"
        )

    def test_sort_by_best_position(self):
        """Test sort_by=best_position sorts networks correctly"""
        response = requests.get(
            f"{BASE_URL}/api/v3/networks?sort_by=best_position", headers=self.headers
        )
        assert response.status_code == 200
        ...
        for network in networks:
            ...
            if pos is None:
                ...
                if null_started:
                    pytest.fail(
                        f"Network {network['name']} has position {pos} but appears after null positions"
                    )
                assert (
                    pos >= prev_position
    ...
    def test_sort_by_ranking_nodes(self):
        ...
        response = requests.get(
            f"{BASE_URL}/api/v3/networks?sort_by=ranking_nodes", headers=self.headers
        )
        assert response.status_code == 200
        ...
        print(
            f"✓ Sort by ranking_nodes works correctly ({len(networks)} networks, descending)"
        )
    ...
    def test_ranking_status_computation_logic(self):
        ...
        for network in networks:
            ...
            if ranking_status == "ranking":
                ...
                assert (
                    ranking_nodes >= 1 or best_pos is not None
