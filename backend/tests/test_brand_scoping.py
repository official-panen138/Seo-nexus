...
BASE_URL = os.environ.get('REACT_APP_BACKEND_URL', 'https://audit-chain-test.preview.emergentagent.com')
...
class TestBrandScoping:
    ...
    def test_super_admin_has_null_brand_scope_ids(self, admin_headers):
        ...
        assert data["brand_scope_ids"] is None, "Super Admin must have null brand_scope_ids"
    ...
    def test_super_admin_can_access_any_brand(self, admin_headers, brands):
        ...
        for brand in brands[:2]:
            response = requests.get(f"{BASE_URL}/api/brands/{brand['id']}", headers=admin_headers)
            assert response.status_code == 200
    ...
    def test_brand_archive_functionality(self, admin_headers, brands):
        ...
        # Archive brand
        response = requests.post(f"{BASE_URL}/api/brands/{brand_id}/archive", headers=admin_headers)
        assert response.status_code == 200
        assert response.json()["status"] == "archived"
        
        # Verify archived
        response = requests.get(f"{BASE_URL}/api/brands/{brand_id}", headers=admin_headers)
        assert response.status_code == 200
        assert response.json()["status"] == "archived"
        
        # Unarchive brand
        response = requests.post(f"{BASE_URL}/api/brands/{brand_id}/unarchive", headers=admin_headers)
        assert response.status_code == 200
    ...
    def test_cannot_archive_already_archived_brand(self, admin_headers, brands):
        ...
        # Archive first
        requests.post(f"{BASE_URL}/api/brands/{brand_id}/archive", headers=admin_headers)
        ...
        # Cleanup: Unarchive
        requests.post(f"{BASE_URL}/api/brands/{brand_id}/unarchive", headers=admin_headers)
    ...
    def test_super_admin_gets_null_brand_scope_on_update(self, admin_headers, brands):
        ...
        response = requests.post(
            ...
            json={"email": test_email, "name": "Test", "password": "test123", "role": "viewer"}
        )
        assert response.status_code == 200
    ...
    def test_v3_asset_domains_filtered_by_brand_scope(self, admin_headers, brands):
        ...
        response = requests.get(f"{BASE_URL}/api/v3/asset-domains", headers=admin_headers)
        assert response.status_code == 200
    ...
    def test_create_asset_domain_requires_valid_brand(self, admin_headers):
        ...
        response = requests.post(
            ...
            headers=admin_headers,
            json={"domain_name": "test-invalid-brand.com", "brand_id": "invalid-brand-id"}
        )
        assert response.status_code == 400
    ...
    def test_create_asset_domain_with_valid_brand(self, admin_headers, brands):
        ...
        requests.delete(f"{BASE_URL}/api/v3/asset-domains/{asset_id}", headers=admin_headers)
    ...
    def test_limited_user_403_for_unauthorized_brand(self, admin_headers, brands):
        ...
        response = requests.post(
            f"{BASE_URL}/api/auth/register",
            json={"email": test_email, "name": "Limited", "password": "test123", "role": "viewer"}
        )
        assert response.status_code == 200
        user_id = response.json()["user"]["id"]
        ...
        try:
            ...
            limited_headers = {"Authorization": f"Bearer {limited_token}", "Content-Type": "application/json"}
            ...
            response = requests.get(f"{BASE_URL}/api/brands/{brand_2}", headers=limited_headers)
    ...
    def test_limited_user_sees_only_scoped_assets(self, admin_headers, brands):
        ...
        try:
            ...
            response = requests.get(f"{BASE_URL}/api/v3/asset-domains", headers=limited_headers)
            assert response.status_code == 200
            assets = response.json()
            
            # Verify all returned assets are from brand_1
            for asset in assets:
                assert asset["brand_id"] == brand_1, f"Asset {asset['domain_name']} should be from brand {brand_1}"
            
            # Get networks - should only return brand_1 networks
            response = requests.get(f"{BASE_URL}/api/v3/networks", headers=limited_headers)
            assert response.status_code == 200
            networks = response.json()
            
            for network in networks:
                assert network["brand_id"] == brand_1, f"Network {network['name']} should be from brand {brand_1}"
