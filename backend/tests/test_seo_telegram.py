...
if not BASE_URL:
    BASE_URL = "https://access-control-hub-44.preview.emergentagent.com"
...
class TestSeoTelegramSettings:
    ...
    def test_seo_telegram_test_notification(self, auth_headers):
        ...
        response = requests.post(
            f"{BASE_URL}/api/v3/settings/telegram-seo/test",
            headers=auth_headers
        )
        assert response.status_code == 200
        data = response.json()
        assert "message" in data
        assert "success" in data["message"].lower() or "berhasil" in data["message"].lower()
...
class TestChangeNoteValidation:
    ...
    def test_create_structure_entry_note_too_short(self, auth_headers):
        ...
        assert response.status_code == 422, f"Expected 422 for short change_note, got {response.status_code}: {response.text}"
...
class TestChangeNoteSuccess:
    ...
    def test_create_and_delete_with_valid_note(self, auth_headers):
        ...
        delete_response = requests.delete(
            ...
            headers=auth_headers,
            json={"change_note": "Cleanup test entry - removing test node after validation"}
        )
