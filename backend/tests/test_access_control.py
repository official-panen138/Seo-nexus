...
class TestUserSearchAPI:
    ...
    def test_search_requires_auth(self):
        ...
        print(f"PASS: User search requires authentication (status: {response.status_code})")
    ...
    def test_search_requires_min_2_chars(self, api_client):
        ...
        assert response.status_code == 422, f"Expected 422 for too short query, got {response.status_code}"
    ...
    def test_search_max_10_results(self, api_client):
        ...
        if response.status_code == 200:
            ...
            print(f"PASS: Search returns max 10 results (got {len(data.get('results', []))})")
...
class TestGetAccessControl:
    ...
    def test_get_access_control_requires_auth(self):
        ...
        response = requests.get(f"{BASE_URL}/api/v3/networks/{TEST_NETWORK_ID}/access-control")
        assert response.status_code in [401, 403], f"Expected 401/403, got {response.status_code}"
        print(f"PASS: Get access control requires authentication (status: {response.status_code})")
    ...
    def test_get_access_control_returns_settings(self, api_client):
        ...
        print(f"PASS: Access control settings returned: visibility_mode={data['visibility_mode']}")
    ...
    def test_get_access_control_404_for_nonexistent_network(self, api_client):
        ...
        response = api_client.get(f"{BASE_URL}/api/v3/networks/nonexistent-id/access-control")
...
class TestUpdateAccessControl:
    ...
    def test_update_access_control_requires_auth(self):
        ...
        assert response.status_code in [401, 403], f"Expected 401/403, got {response.status_code}"
        print(f"PASS: Update access control requires authentication (status: {response.status_code})")
    ...
    def test_update_to_brand_based(self, api_client):
        ...
        get_response = api_client.get(f"{BASE_URL}/api/v3/networks/{TEST_NETWORK_ID}/access-control")
    ...
    def test_update_to_restricted_with_users(self, api_client):
        ...
        assert response.status_code == 200, f"Expected 200, got {response.status_code}: {response.text}"
        print(f"PASS: Updated visibility_mode to restricted with user {current_user_id}")
        ...
        print(f"PASS: Verified visibility_mode is restricted with allowed_user_ids: {data['allowed_user_ids']}")
...
class TestRestrictedNetworkEnforcement:
    ...
    def test_restricted_network_blocks_non_allowed_user(self, api_client):
        ...
        network_response = api_client.get(f"{BASE_URL}/api/v3/networks/{TEST_NETWORK_ID}")
        
        if user_role == "super_admin":
            # Super admin always has access
            assert network_response.status_code == 200, f"Super admin should access restricted network"
...
class TestIntegration:
    ...
    def test_full_access_control_flow(self, api_client):
        ...
        if users:
            ...
            print(f"Step 3: Verified - mode: {data['visibility_mode']}, users: {len(data['allowed_users'])}")
