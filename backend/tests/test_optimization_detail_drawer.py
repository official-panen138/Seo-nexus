...
class TestOptimizationDetailEndpoint(TestSetup):
    ...
    def test_get_detail_not_found(self, session, auth_headers):
        ...
        response = session.get(
            f"{BASE_URL}/api/v3/optimizations/{fake_id}/detail", headers=auth_headers
        )
        assert response.status_code == 404, f"Expected 404, got {response.status_code}"
        print("✓ Returns 404 for non-existent optimization")

    def test_get_existing_optimization_detail(
        self, session, auth_headers, test_network_id
    ):
        ...
        # Get detail
        response = session.get(
            f"{BASE_URL}/api/v3/optimizations/{opt_id}/detail", headers=auth_headers
        )
...
class TestTeamResponseEndpoint(TestSetup):
    ...
    def test_response_validation_min_length(
        self, session, auth_headers, optimization_with_complaint
    ):
        ...
        assert "20" in response.json().get(
            "detail", ""
        ), "Error should mention 20 chars"
        print("✓ Response validation rejects notes < 20 chars")

    def test_response_validation_max_length(
        self, session, auth_headers, optimization_with_complaint
    ):
        ...
        assert response.status_code == 400, f"Expected 400, got {response.status_code}"
        assert "2000" in response.json().get(
            "detail", ""
        ), "Error should mention 2000 chars"
        print("✓ Response validation rejects notes > 2000 chars")

    def test_add_valid_response(
        self, session, auth_headers, optimization_with_complaint
    ):
        ...
        response = session.post(
            f"{BASE_URL}/api/v3/optimizations/{optimization_with_complaint}/responses",
            headers=auth_headers,
            json={"note": test_note, "report_urls": ["https://example.com/evidence"]},
        )
...
class TestResolveComplaintEndpoint(TestSetup):
    ...
    @pytest.fixture(scope="class")
    def optimization_with_active_complaint(
        self, session, auth_headers, test_network_id
    ):
    ...
    def test_resolve_requires_super_admin(self, session, auth_headers):
        ...
        response = session.patch(
            ...
            headers=auth_headers,
            json={
                "resolution_note": "Test resolution note for testing purposes",
                "mark_optimization_complete": False,
            },
        )
    ...
    def test_resolve_validation_min_note(
        self, session, auth_headers, optimization_with_active_complaint
    ):
...
class TestCloseOptimizationEndpoint(TestSetup):
    ...
    def closeable_optimization(self, session, auth_headers, test_network_id):
        ...
        for opt in list_response.json()["data"]:
            if (
                opt.get("complaint_status") in ["none", "resolved"]
                and opt.get("status") != "completed"
            ):
                return opt["id"]
        return None
    ...
    def test_close_blocked_by_complaints(
        self, session, auth_headers, blocked_optimization
    ):
        ...
        ), f"Expected 400 (blocked), got {response.status_code}"
        assert (
            "unresolved" in response.json().get("detail", "").lower()
            or "cannot close" in response.json().get("detail", "").lower()
...
class TestComplaintStatusBadge(TestSetup):
    ...
    def test_optimizations_include_complaint_status(
        self, session, auth_headers, test_network_id
    ):
        ...
        for opt in optimizations:
            assert (
                "complaint_status" in opt
            ), f"Missing complaint_status in optimization {opt['id']}"
            assert opt["complaint_status"] in [
                "none",
                "complained",
                "under_review",
                "resolved",
            ], f"Invalid complaint_status: {opt['complaint_status']}"
