...
class TestV3AssetDomains:
    ...
    def test_get_asset_domain_by_id(self, auth_headers):
        ...
        response = requests.get(
            f"{BASE_URL}/api/v3/asset-domains/{domain_id}", headers=auth_headers
        )
        assert response.status_code == 200, f"Failed: {response.text}"
        data = response.json()
    ...
    def test_asset_domain_not_found(self, auth_headers):
        ...
        response = requests.get(
            f"{BASE_URL}/api/v3/asset-domains/nonexistent-id-123", headers=auth_headers
        )
    ...
    def test_filter_by_status(self, auth_headers):
        ...
        response = requests.get(
            f"{BASE_URL}/api/v3/asset-domains?status=active", headers=auth_headers
        )
        ...
        for domain in data:
            ...
            ), f"Expected active, got {domain['status']}"
...
class TestV3Networks:
    ...
    def test_get_all_networks(self, auth_headers):
        ...
        if len(data) > 0:
            ...
            print(
                f"First network: {network['name']} - {network['domain_count']} domains"
            )
    ...
    def test_get_network_detail_with_tiers(self, auth_headers):
        ...
        response = requests.get(
            f"{BASE_URL}/api/v3/networks/{network_id}", headers=auth_headers
        )
        assert response.status_code == 200, f"Failed: {response.text}"
        data = response.json()
        ...
        for entry in data["entries"][:5]:  # Check first 5
            ...
            print(
                f"  - {entry.get('domain_name', 'N/A')}: Tier {entry.get('calculated_tier', 'N/A')} ({entry.get('tier_label', 'N/A')})"
            )
    ...
    def test_get_network_tiers(self, auth_headers):
        ...
        main_network = next(
            (n for n in networks if "Main" in n.get("name", "")), networks[0]
        )
...
class TestV3Structure:
    ...
    def test_get_structure_entries(self, auth_headers):
        ...
        response = requests.get(f"{BASE_URL}/api/v3/structure", headers=auth_headers)
        assert response.status_code == 200, f"Failed: {response.text}"
        data = response.json()
        assert isinstance(data, list)
        ...
        if len(data) > 0:
            ...
            print(
                f"First entry: {entry.get('domain_name', 'N/A')} - Role: {entry['domain_role']}"
            )
    ...
    def test_filter_by_network(self, auth_headers):
        ...
        network_id = networks[0]["id"]
        response = requests.get(
            f"{BASE_URL}/api/v3/structure?network_id={network_id}", headers=auth_headers
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_filter_by_domain_role(self, auth_headers):
        ...
        response = requests.get(
            f"{BASE_URL}/api/v3/structure?domain_role=main", headers=auth_headers
        )
        assert response.status_code == 200
        data = response.json()
...
class TestV3Reports:
    ...
    def test_dashboard_stats(self, auth_headers):
        ...
        response = requests.get(
            f"{BASE_URL}/api/v3/reports/dashboard", headers=auth_headers
        )
        assert response.status_code == 200, f"Failed: {response.text}"
        data = response.json()
        ...
        print(
            f"  Structure Entries: {data['collections'].get('seo_structure_entries', 0)}"
        )
    ...
    def test_conflicts_detection(self, auth_headers):
        ...
        for conflict in data["conflicts"][:3]:
            print(
                f"  - {conflict.get('domain_name', 'N/A')}: {conflict.get('type', 'N/A')}"
            )
...
class TestV3ActivityLogs:
    ...
    def test_get_activity_logs(self, auth_headers):
        ...
        response = requests.get(
            f"{BASE_URL}/api/v3/activity-logs", headers=auth_headers
        )
        assert response.status_code == 200, f"Failed: {response.text}"
        data = response.json()
        assert isinstance(data, list)
        ...
        if len(data) > 0:
            ...
            print(
                f"Latest log: {log['actor']} - {log['action_type']} - {log['entity_type']}"
            )
    ...
    def test_activity_log_stats(self, auth_headers):
        ...
        response = requests.get(
            f"{BASE_URL}/api/v3/activity-logs/stats", headers=auth_headers
        )
        assert response.status_code == 200, f"Failed: {response.text}"
        data = response.json()
...
class TestV3DataIntegrity:
    ...
    def test_migrated_data_counts(self, auth_headers):
        ...
        structure_response = requests.get(
            f"{BASE_URL}/api/v3/structure", headers=auth_headers
        )
