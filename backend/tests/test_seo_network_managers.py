...
BASE_URL = os.environ.get(
    "REACT_APP_BACKEND_URL", "https://conflict-resolver-ui.preview.emergentagent.com"
)
...
class TestGetNetworkManagers:
    ...
    def test_get_managers_returns_is_current_user_manager_flag(
        ...
        assert (
            "is_current_user_manager" in data
        ), "Missing is_current_user_manager field"
        assert isinstance(
            data["is_current_user_manager"], bool
        ), "is_current_user_manager should be boolean"
    ...
    def test_get_managers_returns_visibility_mode(self, authenticated_client):
        ...
        assert data["visibility_mode"] in [
            "brand_based",
            "restricted",
            "public",
    ...
    def test_get_managers_404_for_invalid_network(self, authenticated_client):
        ...
        response = authenticated_client.get(
            f"{BASE_URL}/api/v3/networks/{fake_id}/managers"
        )
...
class TestUpdateNetworkManagers:
    ...
    def test_update_managers_requires_super_admin(self, api_client):
        ...
        ), f"Super Admin update failed: {response.text}"
    ...
    def test_update_managers_returns_summary_cache(
        ...
        assert (
            "manager_summary_cache" in data
        ), "Missing manager_summary_cache in response"
        if data["manager_summary_cache"]:
...
class TestLegacyAccessControlEndpoints:
    ...
    def test_put_access_control_redirects_to_managers(self, authenticated_client):
        ...
        assert (
            response.status_code == 200
        ), f"Legacy PUT endpoint failed: {response.text}"
...
class TestManagerAuditLogs:
    ...
    def test_get_managers_audit_logs_requires_super_admin(self, authenticated_client):
        ...
        response = authenticated_client.get(
            f"{BASE_URL}/api/v3/networks/{TEST_NETWORK_ID}/managers-audit-logs"
        )
