...
class TestSeoChangeIntelligence:
    ...
    def test_structure_create_requires_change_note(self, api_client, auth_token):
        ...
        domains = api_client.get(f"{BASE_URL}/api/v3/networks/{network['id']}/available-domains").json()
        ...
        assert response.status_code in [400, 422], f"Expected 400/422 when change_note missing, got {response.status_code}"
    ...
    def test_structure_create_with_change_note_succeeds(self, api_client, auth_token):
        ...
        assert response.status_code in [200, 201], f"Structure create with change_note failed: {response.text}"
    ...
    def test_structure_create_change_note_too_short(self, api_client, auth_token):
        ...
        assert response.status_code in [400, 422], f"Expected 400/422 for short change_note, got {response.status_code}"
    ...
    def test_structure_update_requires_change_note(self, api_client, auth_token):
        ...
        response = api_client.put(f"{BASE_URL}/api/v3/structure/{entry_id}", json=update_payload)
    ...
    def test_structure_update_with_change_note_succeeds(self, api_client, auth_token):
        ...
        assert response.status_code == 200, f"Structure update with change_note failed: {response.text}"
        
        data = response.json()
        assert data.get("index_status") == "noindex", "Index status should be updated to noindex"
    ...
    def test_structure_delete_with_change_note_succeeds(self, api_client, auth_token):
        ...
        delete_payload = {
            "change_note": "Deleting test node - cleanup after testing"
        }
        ...
        assert response.status_code == 200, f"Structure delete with change_note failed: {response.text}"
    ...
    def test_get_network_change_history(self, api_client, auth_token):
        ...
        response = api_client.get(f"{BASE_URL}/api/v3/networks/{network_id}/change-history")
        
        assert response.status_code == 200, f"Get change history failed: {response.text}"
        ...
        if logs:
            ...
            assert "actor_email" in log or "actor_user_id" in log, "Log should have actor info"
            print(f"✓ Change history has {len(logs)} logs with proper structure")
        else:
            print(f"✓ Change history endpoint works (empty logs - expected if no prior changes)")
    ...
    def test_change_history_not_found_for_invalid_network(self, api_client, auth_token):
        ...
        response = api_client.get(f"{BASE_URL}/api/v3/networks/invalid-network-id-12345/change-history")
        
        assert response.status_code == 404, f"Expected 404 for invalid network, got {response.status_code}"
    ...
    def test_get_network_notifications(self, api_client, auth_token):
        ...
        response = api_client.get(f"{BASE_URL}/api/v3/networks/{network_id}/notifications")
        ...
        if notifications:
            ...
            print(f"✓ Notifications endpoint works (empty - expected if no important events)")
    ...
    def test_mark_notification_read(self, api_client, auth_token):
        ...
        if not notification_id:
            ...
            assert response.status_code == 404, f"Expected 404 for non-existent notification"
    ...
    def test_get_change_log_stats(self, api_client, auth_token):
        ...
        assert response.status_code == 200, f"Get change log stats failed: {response.text}"
        
        stats = response.json()
        
        # Verify expected fields
        assert "period_days" in stats or "total_changes" in stats, \
            "Stats should have period or total_changes field"
        
        if "changes_by_user" in stats:
            assert isinstance(stats["changes_by_user"], list), "changes_by_user should be list"
        
        if "changes_by_network" in stats:
            assert isinstance(stats["changes_by_network"], list), "changes_by_network should be list"
    ...
    def test_change_log_stats_with_days_param(self, api_client, auth_token):
        ...
        assert response.status_code == 200, f"Get stats with days param failed: {response.text}"
        
        stats = response.json()
        assert stats.get("period_days") == 7 or "period_days" not in stats, \
    ...
    def test_get_telegram_seo_settings(self, api_client, auth_token):
        ...
        assert response.status_code == 200, f"Get SEO telegram settings failed: {response.text}"
        
        settings = response.json()
        
        # Either configured: true/false or just returns settings
        assert "configured" in settings or isinstance(settings, dict), \
            "Response should indicate configuration status"
        
        print(f"✓ SEO Telegram settings: configured={settings.get('configured', 'unknown')}")
    ...
    def test_update_telegram_seo_settings(self, api_client, auth_token):
        ...
        assert response.status_code == 200, f"Update SEO telegram settings failed: {response.text}"
        ...
        assert settings.get("configured") == True or settings.get("chat_id"), \
