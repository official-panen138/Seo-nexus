...
async def validate_prerequisites(db) -> Dict[str, Any]:
    ...
    if source_count == 0:
        issues.append(
            "Source collection 'groups' is empty (may be OK if no networks exist)"
        )
    ...
    if target_count > 0:
        issues.append(
            f"Target collection 'seo_networks' already has {target_count} documents"
        )
...
async def main():
    parser = argparse.ArgumentParser(
        description="Phase 3 Migration: Groups to SeoNetworks"
    )
    parser.add_argument(
