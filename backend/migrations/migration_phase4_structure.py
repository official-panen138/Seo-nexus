...
async def load_mappings(db) -> Dict[str, Dict[str, str]]:
    ...
    assets = await db.asset_domains.find({}, {"_id": 0, "id": 1, "legacy_id": 1}).to_list(100000)
    for asset in assets:
        if asset.get("legacy_id"):
            mappings["domains"][asset["legacy_id"]] = asset["id"]
    
    # Load network mappings from seo_networks
    networks = await db.seo_networks.find({}, {"_id": 0, "id": 1, "legacy_id": 1}).to_list(10000)
...
async def validate_prerequisites(db) -> Dict[str, Any]:
    ...
    if target_count > 0:
        issues.append(f"Target collection 'seo_structure_entries' already has {target_count} documents")
    ...
    domains = await db.domains.find(
        {"parent_domain_id": {"$ne": None}},
        {"_id": 0, "id": 1, "parent_domain_id": 1}
    ).to_list(100000)
    
    all_domain_ids = set(d["id"] for d in await db.domains.find({}, {"_id": 0, "id": 1}).to_list(100000))
    ...
    if orphan_refs:
        warnings.append(f"Found {len(orphan_refs)} domains with invalid parent references")
...
async def dry_run(db) -> Dict[str, Any]:
    ...
    for domain in domains:
        ...
        if entry:
            ...
            print(f"       has_target: {'Yes' if entry['target_asset_domain_id'] else 'No'}")
...
async def main():
    parser = argparse.ArgumentParser(description="Phase 4 Migration: SEO Structure Entries")
