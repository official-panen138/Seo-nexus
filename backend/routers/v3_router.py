...
async def get_asset_domains(
    ...
    if sort_by == "critical":
        ...
        db_field = sort_field_map[sort_by]
        # Only add secondary sort if it's a different field
        if db_field == "domain_name":
...
async def get_seo_monitoring_coverage(
    ...
    domains_in_seo = await db.asset_domains.find(
        ...
        {"_id": 0, "id": 1, "domain_name": 1, "monitoring_enabled": 1, 
         "lifecycle_status": 1, "quarantine_category": 1, "expiration_date": 1}
    ).to_list(100000)
    
    total_in_seo = len(domains_in_seo)
    
    # Count by monitoring status
    # ONLY active lifecycle domains with domain_active_status=active should be counted
    monitored = 0
    unmonitored = 0
    active_count = 0
    active_monitored = 0
    ...
    for d in domains_in_seo:
        ...
        # Compute domain_active_status
        domain_active_status, _ = compute_domain_active_status(d.get("expiration_date"))
        is_expired = domain_active_status == "expired"
        ...
        # But also exclude expired domains from monitoring counts
        if lifecycle in [DomainLifecycleStatus.ACTIVE.value, None]:
            if is_expired:
                continue  # Expired domains should not be monitored
    ...
    # Calculate coverage percentage (only active lifecycle + non-expired domains are monitorable)
    coverage_percentage = (monitored / active_count * 100) if active_count > 0 else 100.0
    
    # RULE 4: Find root domains missing monitoring (domains used via path but root not monitored)
    ...
    if root_domains_with_paths:
        root_domain_docs = await db.asset_domains.find(
            ...
            {"_id": 0, "id": 1, "monitoring_enabled": 1, "lifecycle_status": 1, "quarantine_category": 1, "expiration_date": 1}
        ).to_list(10000)
        ...
        for d in root_domain_docs:
            ...
            # Only count as missing if domain is active lifecycle and not expired
            if (not is_monitoring_enabled and 
                not is_quarantined and 
                lifecycle in [DomainLifecycleStatus.ACTIVE.value, None] and
                domain_active_status == "active"):
...
@router.post("/domains/archive-expired")
async def archive_expired_domains(
    background_tasks: BackgroundTasks,
    current_user: dict = Depends(get_current_user_wrapper),
):
    """
    PHASE 6: Auto-archive domains that are expired + not_renewed.
    
    This endpoint:
    1. Finds all domains with lifecycle_status='not_renewed' and domain_active_status='expired'
    2. Sets archived=True on those domains
    3. Logs all transitions
    4. Returns summary of archived domains
    
    Only Super Admin can trigger this action.
    """
    if current_user.get("role") != "super_admin":
        raise HTTPException(status_code=403, detail="Only Super Admin can archive expired domains")
    
    now = datetime.now(timezone.utc)
    now_iso = now.isoformat()
    
    # Find expired domains with not_renewed lifecycle
    # These are domains where expiration_date < now AND lifecycle_status = not_renewed
    expired_not_renewed = await db.asset_domains.find(
        {
            "lifecycle_status": DomainLifecycleStatus.NOT_RENEWED.value,
            "archived": {"$ne": True},
        },
    ...
    # Filter to only include those that are actually expired
    domains_to_archive = []
    for domain in expired_not_renewed:
        exp_date = domain.get("expiration_date")
        if exp_date:
            try:
                exp_dt = datetime.fromisoformat(exp_date.replace("Z", "+00:00"))
                if exp_dt.date() < now.date():
                    domains_to_archive.append(domain)
            except (ValueError, TypeError):
                continue
    ...
    if not domains_to_archive:
        return {
            "success": True,
            "message": "No expired+not_renewed domains to archive",
            "archived_count": 0,
            "domains": []
        }
    
    archived_domains = []
    for domain in domains_to_archive:
        ...
        await db.asset_domains.update_one(
            {"id": domain_id},
            {
                "$set": {
                    "archived": True,
                    "archived_at": now_iso,
                    "archived_reason": "Auto-archived: expired+not_renewed",
    ...
    return {
        "success": True,
        "message": f"Successfully archived {len(archived_domains)} expired+not_renewed domains",
        "archived_count": len(archived_domains),
        "archived_at": now_iso,
        "domains": archived_domains
    }


@router.get("/domains/archived")
async def get_archived_domains(
    page: int = Query(default=1, ge=1),
    ...
    """
    PHASE 6: Get list of archived domains.
    
    Archived domains are:
    - Excluded from active dashboards
    - Excluded from monitoring
    - Excluded from SEO Network node selection
    - Read-only (can only be restored by Super Admin)
    """
    # Build query
    query = build_brand_filter(current_user)
    query["archived"] = True
    ...
    skip = (page - 1) * limit
    domains = await db.asset_domains.find(query, {"_id": 0}).skip(skip).limit(limit).to_list(limit)
    
    # Enrich with brand names
    for domain in domains:
        if domain.get("brand_id"):
            brand = await db.brands.find_one({"id": domain["brand_id"]}, {"_id": 0, "name": 1})
            domain["brand_name"] = brand.get("name") if brand else "Unknown"
...
@router.post("/asset-domains/{asset_id}/restore")
async def restore_archived_domain(
    asset_id: str,
    current_user: dict = Depends(get_current_user_wrapper),
):
    """
    PHASE 6: Restore an archived domain - SUPER ADMIN ONLY.
    
    This endpoint:
    1. Removes archived flag
    2. Sets lifecycle back to 'active' (if not expired) or 'not_renewed' (if still expired)
    3. Logs the transition
    
    Auto-restore is also triggered when expiration_date is updated to a future date.
    """
    if current_user.get("role") != "super_admin":
        raise HTTPException(status_code=403, detail="Only Super Admin can restore archived domains")
    ...
    if not existing.get("archived"):
        raise HTTPException(status_code=400, detail="Domain is not archived")
    
    now = datetime.now(timezone.utc)
    now_iso = now.isoformat()
    
    # Determine new lifecycle based on expiration
    domain_active_status, _ = compute_domain_active_status(existing.get("expiration_date"))
    
    if domain_active_status == "expired":
        new_lifecycle = DomainLifecycleStatus.NOT_RENEWED.value
    else:
    ...
    if activity_log_service:
        await activity_log_service.log(
            ...
            after_value={**existing, **update_dict},
            metadata={
                "notes": f"Domain restored from archive. New lifecycle: {new_lifecycle}",
...
async def auto_restore_domain_on_expiration_update(domain_id: str, new_expiration_date: str, actor_email: str):
    """
    PHASE 6: Auto-restore an archived domain when expiration date is updated to future.
    
    Called automatically when asset domain is updated with a new expiration date.
    """
    domain = await db.asset_domains.find_one({"id": domain_id}, {"_id": 0})
    if not domain or not domain.get("archived"):
        return None
    
    # Check if new expiration is in the future
    try:
        new_exp_dt = datetime.fromisoformat(new_expiration_date.replace("Z", "+00:00"))
        if new_exp_dt.date() <= datetime.now(timezone.utc).date():
            return None  # Still expired, don't restore
    except (ValueError, TypeError):
        return None
    ...
    update_dict = {
        ...
        "restored_reason": "Auto-restored: expiration date updated to future",
        "updated_at": now_iso
    }
    ...
    if activity_log_service:
        await activity_log_service.log(
            ...
            entity_id=domain_id,
            before_value=domain,
            after_value={**domain, **update_dict},
            metadata={
                "notes": f"Auto-restored: expiration date updated from {domain.get('expiration_date')} to {new_expiration_date}",
                "transition": "archived → active",
                "trigger": "auto_restore_expiration_update"
            }
        )
...
@router.get("/domains/archive-candidates")
async def get_archive_candidates(
    current_user: dict = Depends(get_current_user_wrapper),
):
    """
    PHASE 6: Get domains that are candidates for archiving.
    
    Returns domains with:
    - lifecycle_status = 'not_renewed'
    - domain_active_status = 'expired' (expiration_date < today)
    ...
    domains = await db.asset_domains.find(query, {"_id": 0}).to_list(10000)
        ...
        if exp_date:
            try:
                ...
                if exp_dt.date() < now.date():
                    ...
                    days_expired = (now.date() - exp_dt.date()).days
                    domain["days_expired"] = days_expired
